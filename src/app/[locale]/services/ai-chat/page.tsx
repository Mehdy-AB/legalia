...
export default function AiChatPage() {
    ...
    const functionKeys = ['routing', 'quality', 'crossCheck', 'privacy'] as const
    const functionIcons = {
        routing: Network,
        quality: ShieldCheck,
        crossCheck: GitMerge,
    ...
    const benefitKeys = ['multiModel', 'accuracy', 'review', 'security', 'experience'] as const
    const benefitIcons = {
        multiModel: Layers,
        accuracy: Zap,
        review: CheckCircle,
    ...
    return (
        <main className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300">
            {/* Hero Section */}
            <PageHero className="text-white" videoSrc="https://res.cloudinary.com/dywcowkoy/video/upload/v1768749672/images/ai-chat.mp4">
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {functionKeys.map((key) => {
                            ...
                            return (
                                <div key={key} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 text-center hover:shadow-md transition-all">
                                    <div className="w-14 h-14 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <Icon className="w-7 h-7 text-blue-600 dark:text-blue-400" />
                                    </div>
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t(`functions.${key}.title`)}</h3>
                                    <p className="text-gray-600 dark:text-gray-400 text-sm">{t(`functions.${key}.desc`)}</p>
                                </div>
                            )
                        })}
                    </div>
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="space-y-6">
                            {(t.raw('howItWorksSteps') as string[]).map((step, index) => (
                                <div
                                    key={index}
                                    className="flex items-start gap-6 bg-slate-50 dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all duration-300 border-s-4 border-s-amber-400"
                                >
            ...
            <section className="py-20 bg-gradient-to-br from-slate-900 to-blue-900">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-12">
                        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t('benefits')}</h2>
                        ...
                        {benefitKeys.map((key) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <h3 className="text-xl font-bold text-white mb-2">{t(`benefitsList.${key}.title`)}</h3>
                                    <p className="text-gray-300">{t(`benefitsList.${key}.desc`)}</p>
                                </div>
                            )
                        })}
                    </div>
