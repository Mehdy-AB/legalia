...
import { Video, Eye, Layers, TestTube, Lightbulb, AlertTriangle, Users, Car, Building, Clock, GraduationCap } from 'lucide-react'
import { useTranslations } from 'next-intl'

export default function VisualFactsPage() {
    const t = useTranslations('VisualFactsPage')

    const featureKeys = ['dataVisual', 'deepUnderstanding', 'testScenarios', 'visualEvidence', 'contradictions', 'support'] as const
    const featureIcons = {
        dataVisual: Layers,
        deepUnderstanding: Eye,
        testScenarios: TestTube,
        visualEvidence: Lightbulb,
        contradictions: AlertTriangle,
        support: Users
    }

    const useCaseKeys = ['traffic', 'realEstate', 'timeline', 'educational', 'visual'] as const
    const useCaseIcons = {
        traffic: Car,
        realEstate: Building,
        timeline: Clock,
    ...
    return (
        <main className="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-300">
            {/* Hero Section */}
            <PageHero className="text-white" videoSrc="https://res.cloudinary.com/dywcowkoy/video/upload/v1768750089/images/visual-facts.mp4">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-xl text-gray-200 leading-relaxed font-light max-w-3xl mx-auto">
                        {t('subtitle')}
                        <br />
                        <span className="block mt-4 text-amber-400 font-semibold">{t('subtitleHighlight')}</span>
                    </p>
                </div>
            </PageHero>
                    ...
                    <div className="max-w-4xl mx-auto">
                        <div className="bg-gradient-to-br from-amber-50 to-blue-50 dark:from-amber-900/20 dark:to-blue-900/20 rounded-2xl p-8 md:p-12 border border-amber-100 dark:border-amber-900/30 shadow-lg">
                            ...
                            <div className="flex justify-center mb-8">
                                <img src="/images/visual.bmp" alt="Visual Facts" className="w-full max-w-2xl h-auto object-cover rounded-xl shadow-md" />
                            </div>
                            <p className="text-lg text-gray-700 dark:text-gray-300 leading-loose mb-6">{t('mainDesc')}</p>
                            <p className="text-lg text-gray-700 dark:text-gray-300 leading-loose mb-6">{t('mainDesc2')}</p>
                        </div>
                    </div>
                </div>
            </section>
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="space-y-6">
                            {(t.raw('howItWorksSteps') as string[]).map((step, index) => (
                                <div key={index} className="flex items-start gap-6 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all duration-300">
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {featureKeys.map((key) => {
                            ...
                            return (
                                <div key={key} className="group bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 relative overflow-hidden">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t(`featuresList.${key}.title`)}</h3>
                                    <p className="text-gray-600 dark:text-gray-400">{t(`featuresList.${key}.desc`)}</p>
                                </div>
                            )
                        })}
                    </div>
            ...
            <section className="py-20 bg-gradient-to-br from-slate-900 to-blue-900">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-12">
                        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t('useCases')}</h2>
                            ...
                            return (
                                <div key={key} className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/20 hover:bg-white/20 transition-all duration-300">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-bold text-white mb-1">{t(`useCasesList.${key}`)}</h3>
                                        </div>
                                    </div>
                                </div>
                            )
