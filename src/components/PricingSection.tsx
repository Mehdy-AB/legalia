        ...
        <div className="text-center mb-16">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10 mb-10">
            ...
            <div className="flex items-center gap-4" dir="ltr">
              <span className={`text-lg font-medium ${billingCycle === 'monthly' ? 'text-gray-900' : 'text-gray-500'}`}>{t('monthly')}</span>
              <button
            ...
            return (
              ...
              >
                ...
                <div className={`text-center mb-6 pt-4`}>
                  <h3 className="text-xl font-bold text-gray-900 mb-2">{t(`${plan.key}.name`)}</h3>
                  <div className="flex items-center justify-center gap-1 mb-2">
                    <span className="text-4xl font-extrabold text-gray-900">{calculateFinalPrice(plan, originalIndex)}</span>
                    <span className="text-gray-500 text-sm">{currentCurrency.symbol} {t('perMonth')}</span>
                  </div>
                  <p className="text-gray-500 text-sm">{t(`${plan.key}.description`)}</p>
                </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {extraCredits.map((item, index) => (
              <div key={index} className="flex flex-col items-center p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-amber-300 transition-colors">
                <span className="text-lg font-bold text-gray-700 mb-2">{item.credits} {t('credits')}</span>
